...
**使用 x-tweet-fetcher 抓取推文时：**
- ✅ 使用 FxTwitter 公共API，**无需登录/无需API Key**
- ✅ 监控竞品/研究时**不暴露自己账号身份**
- ⚠️ 发推/互动时仍需使用 twclaw（会显示账号）

**切换规则：**
| 场景 | 工具 | 隐私级别 |
|------|------|----------|
| 监控/研究/抓取竞品 | x-tweet-fetcher | ✅ 完全匿名 |
| 发推/点赞/互动 | twclaw | ⚠️ 显示账号 |
...
| @笔仙 | 只生成内容草稿，不直接发外网 |
| @雷达 | 只读取公开信息，用x-tweet-fetcher保护隐私 |
| @数据帝 | 只分析已有数据，不下载敏感文件 |
| @小助 | 综合协调，不做敏感操作 |
...
| 2026-02-22 | 添加推特操作隐私保护规则（x-tweet-fetcher） |
